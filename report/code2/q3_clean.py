...
SLA_L = {"U": 5.0, "E": 100.0, "M": 500.0}
SLA_R_E_MBPS = 50.0
POWER_MIN, POWER_MAX = 10.0, 30.0
...
def build_env() -> Tuple[Env, List[str]]:
    """构建环境和用户列表"""
    time_list, arrivals = load_time_series_csv(os.path.join(ATTACH3_DIR, "taskflow_用户任务流.csv"))
    phi = {}
    h_abs = {}
...
def user_rate_bps(env: Env, bs: str, name: str, cat: str, power_alloc: Dict[str, Dict[str, float]], active_bs_same_slice: List[str], t_ms: int) -> float:
    """计算考虑干扰的下行速率"""
...
def simulate_window_multibs(env: Env, init_states: Dict[str, UserState], mapping: Dict[str, str], rb_alloc: Dict[str, Dict[str, int]], power_alloc: Dict[str, Dict[str, float]], t0: int, copy_state: bool = True):
    """多基站窗口仿真"""
    states = {}
    if copy_state:
            ...
            for s in SLICE_LIST:
                ...
                for u in list(active[bs][s]):
                    ...
                    if head.remain_mbit <= 1e-12:
                        ...
                        if s == 'U':
                            sum_U += ALPHA ** L_ms if L_ms <= SLA_L['U'] else -M_U
                        elif s == 'E':
...
def random_individual(num_users: int):
    """生成随机个体：用户-基站映射 + RB分配 + 功率设置"""
    user_bs = np.random.randint(0, 3, size=num_users, dtype=np.int8)
    rb_counts = np.zeros(6, dtype=np.int16)  # 每个BS的U_RB,E_RB
    idx = 0
    for _bs in BS_LIST:
        total = 50
...
def main():
    """附录精简：第三问省略与第二问相同的MPC仿真，仅保留关键函数与接口。"""
    print("[附录] 问题三：已省略滚动窗口MPC仿真及完整GA循环，仅保留核心组件与接口。")
